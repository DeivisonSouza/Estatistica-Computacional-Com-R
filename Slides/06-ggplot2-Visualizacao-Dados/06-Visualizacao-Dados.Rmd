...
subtitle: "Visualização de Dados com R (ggplot2)"
...
## ᨒ <br> `r anicon::faa("pagelines", animate="horizontal", colour="green")` .font80[Visualização de Dados no R]`r anicon::faa("pagelines", animate="horizontal", colour="green")`
###### (Pacotes: ggplot2 e patchwork)
...
&bull;
<div4>Visualização de Dados no R: ggplot2</div4>
</span>
</div>
...
```
.font65[**Fonte**: WICKHAM, H.; ÇETINKAYA-RUNDEL, M.; GROLEMUND, G. R for Data Science: Import, Tidy, Transform, Visualize, and Model Data. 2. ed. Sebastopol: O'Reilly Media, 2023.]
]
...
.pull-left-4[
```{r, echo=F, eval=F}
# data <- readxl::read_excel(path = "data/IF100.xlsx")
# 
# set.seed(1000)
# 
# data_sample <- data %>% 
...
# 
# writexl::write_xlsx(x = data_sample, "data/sample.xlsx")
# writexl::write_xlsx(x = data_sample2, "data/sample2.xlsx")
```

```{r data, echo=T, eval=F}
data_sample <- readxl::read_excel(path = "data/sample.xlsx")
```
...
ggplot(data = data_sample %>% filter(Nome_Especie == 'Macaranduba'), 
       mapping = aes(x = DAP, y = V)) +           # 1ª camada
...
ggplot(data = data_sample %>% 
           filter(Nome_Especie %in% c('Acapu', 'Macaranduba')), 
       mapping = aes(x = DAP, y = V)) +           # 1ª camada
  geom_point(mapping = aes(colour = Nome_Especie,
                           shape = Selecao),
             size = 4)                            # 2ª camada
...
ggplot(data = data_sample %>% 
           filter(Nome_Especie %in% c('Andiroba', 'Macaranduba')), 
       mapping = aes(x = DAP, y = V)) +               # 1ª camada
  geom_point(mapping = aes(colour = Nome_Especie,
                           shape = Selecao),
             size = 4) +                              # 2ª camada
...
data_sample %>%
  filter(Nome_Especie %in% 'Macaranduba') %>%
...
data <- read_excel(path = "data/IF100.xlsx")
...
labels2 <- c(Acapu = "Vouacapoua", Andiroba = "Carapa",
             `Goiabao` = "Pouteria", `Macaranduba` = "Manilkara")

...
###### .font80[(Pacotes: Tidyverse, summarytools e DataExplorer)]
...
- Compreender os fundamentos da **Análise Exploratória de Dados (AED)**;
- Explorar e interpretar conjuntos de dados **Iris Flower** utilizando a linguagem **R**;
- Produzir estatísticas descritivas, gráficos exploratórios e relatórios automatizados; e
- Utilizar os pacotes **summarytools** e **DataExplorer** para apoiar a exploração inicial dos dados.
...
.pull-left[
.font90[
[1. Introdução à Análise Exploratória de Dados](#iris)
  - Conceitos
  - Objetivos
  - Fluxo de trabalho

[2. Análise Exploratória com o conjunto Iris](#iris)
...
[3. Automatizando a AED com summarytools](#iris)
  - Tabelas de frequências
  - Estatísticas descritivas
  - Relatórios automatizados

[4. Automatizando a AED com DataExplorer](#iris)
  - Inspeção dos dados
  - Valores ausentes
...
---
name: objetivos_aed
## AED: O que investigar nos dados?

.alert2[
.font90[
💡 A AED busca compreender a **estrutura**, a **qualidade** e o **comportamento dos dados**, orientando as etapas seguintes da análise.
]
]
...
- Qual é o tipo de cada variável (numérica, etc.)?
]
]
...
- Existem erros ou inconsistências nos dados?
]
]
...
- Existem padrões ou tendências nos dados?
]
]

---
name: fluxo_aed
## Fluxo de Trabalho da AED
...
.step[
**2. Qualidade**  
Valores ausentes, inconsistências e outliers
]
...
.step[
**3. Exploração**  
Estatísticas descritivas, distribuições e relações
]
...
.alert2[
💡 A AED transforma dados em informações que orientam as próximas etapas da análise.
]
...
Antes de iniciar a AED, organize um projeto (`.Rproj`):
...
<div3>Análise Exploratória de Dados (Módulo 3)</div3>
&bull;
<div4>AED com DataExplorer</div4>
</span>
</div>

---
name: dataexplorer_intro
## Pacote **DataExplorer**

<br>

.font95[
O pacote .orange[**DataExplorer**] reúne funções para **automatizar** diversas etapas da **Análise Exploratória de Dados (AED)**, permitindo explorar rapidamente a estrutura, a qualidade e o comportamento dos dados.
]
...
.font110[
.alert2[
**Poucas linhas de código** → **Tabelas, gráficos e relatórios exploratórios**
]
]
]
...
- Gráficos: Histogramas e densidade
]
]
...
.alert[
💡 O **DataExplorer** complementa os pacotes **skimr** e **summarytools**, adicionando recursos gráficos e relatórios automatizados para a AED.
]

---
name: install_dataexplorer
## Instalando e carregando o pacote

<br>

```{r install_dataexplorer, eval=FALSE}
...
.font95[
Após carregar o pacote, suas funções estarão disponíveis para automatizar diferentes etapas da **Análise Exploratória de Dados (AED)**.
]

<br>

.alert2[
💡 O pacote é compatível com objetos da classe **data.frame** e **tibble**, integrando-se facilmente ao fluxo de trabalho do **Tidyverse**.
]

---
name: introduce_dataexplorer
## Conhecendo o conjunto de dados

<br>

- **Inspeção dos dados**: utilize a função .green[**introduce()**] para obter um resumo geral do conjunto de dados.

<br>

```{r introduce_dataexplorer, echo=TRUE, eval=FALSE}
introduce(data)
```
...
.alert[
💡 A função `plot_intro()` apresenta, em um único painel, informações sobre observações, variáveis, tipos de dados e valores ausentes.
]

```{r plot_intro, echo=TRUE, eval=FALSE}
plot_intro(data)
```
...
```{r ref.label="plot_intro", echo=FALSE, eval=TRUE, fig.align='center', out.width='100%'}
plot_intro(data)
```
...
.alert[
- A função `plot_missing()` resume visualmente os **valores ausentes** de cada variável.
]

```{r plot_missing, echo=TRUE, eval=FALSE}

data |>
  mutate(
    Sepal_Length = replace(Sepal_Length, c(5, 18, 42), NA),
    Species = replace(Species, c(10, 35), NA)
  ) |>
  plot_missing()
...
.alert2[
💡 Valores ausentes foram inseridos artificialmente para ilustrar a função.
]

---
name: plot_missing_result
## Identificando valores ausentes
...
- Existem muitas observações com **valores ausentes**?
- **Quais variáveis** são mais afetadas?
- É necessário **tratar os valores ausentes** antes da análise?
]
]

.pull-right-4[

```{r ref.label="plot_missing", echo=FALSE, eval=TRUE, fig.align='center', out.width='90%'}
...
.alert[
A função .green[**plot_bar()**] produz gráficos de barras para as **variáveis categóricas**.
]
...
.alert2[
💡 Um gráfico é gerado para cada **variável categórica** do conjunto de dados.
]

---
name: plot_bar_result
## Distribuição das variáveis categóricas
...
- Há possíveis **inconsistências** nas categorias?
]
...
```{r ref.label="plot_bar", echo=FALSE, eval=TRUE, fig.align='center', out.width='90%'}
plot_bar(data)
```
...
.lert[
- A função .green[**plot_correlation()**] calcula e representa graficamente a **correlação** entre as variáveis numéricas.
]

<br>

```{r plot_correlation, echo=TRUE, eval=FALSE}
data |>
...
.alert2[
💡 Produz um **mapa de calor** (heatmap) da **correlação de Pearson** entre as variáveis numéricas.
]

---
name: plot_correlation_result
## Correlação entre variáveis
...
- Existem variáveis com **forte associação linear**?
- As associações são **positivas** ou **negativas**?
- Quais pares apresentam os **maiores coeficientes de correlação**?

📌 **Lembrar (Pearson)**
- O sinal (+ ou −) indica a direção da associação linear.
- O valor absoluto de r (|r|) indica a força da associação linear.
- Quanto mais próximo de ±1, mais forte é a associação linear.
- A significância estatística da correlação pode ser avaliada pelo teste t.
]
]
...
```{r ref.label="plot_correlation", echo=FALSE, eval=TRUE, fig.align='center', out.width='90%'}
data |>
  select(where(is.numeric)) |>
  plot_correlation()
```
...
---
name: correlation_causality
## Correlação ≠ Causalidade

<br>

.alert[
- A correlação mede a **intensidade e a direção** associação linear, não a relação de **causa e efeito**.
]
...
- As variáveis **variam juntas**.
- Pode ser **positiva** ou **negativa**.
- Não explica **por que** isso acontece.
]
...
- Alterações em uma variável **provocam mudanças** na outra.
- Requer evidências adicionais, como experimentos ou estudos bem controlados.
]

]

---
name: correlation_causality_iris
## Correlação ≠ Causalidade
...
As variáveis **Petal.Length** e **Petal.Width** apresentam forte correlação positiva (0,96).
]
...
O aumento do **comprimento da pétala** causa o aumento da **largura da pétala**?
]
...
.alert2[
💡 A forte correlação provavelmente decorre do fato de que ambas as variáveis são influenciadas por **fatores biológicos comuns**, e **não porque uma variável causa alterações na outra**.
]

---
name: create_report
## Relatório exploratório automatizado

<br>

.alert[
- A função .green[**create_report()**] gera automaticamente um **relatório HTML** contendo diversas análises exploratórias do conjunto de dados.
]

<br>

```{r create_report, echo=TRUE, eval=FALSE}
data |> 
  create_report(output_file = "Relatorio.html",
                output_dir = "report")
```
...
.alert2[
💡 O relatório reúne tabelas, gráficos e estatísticas em um único documento, facilitando a inspeção inicial dos dados.
]
...
.font90[
- Cada grupo (até 4 pessoas) deverá criar um Projeto (.Rproj) e desenvolver uma AED. Inicialmente, explore os pacotes estudados do **Tidyverse** (**readr**, **dplyr**, **ggplot2**, etc.). Além disso, explore outras bibliotecas especializadas em AED, como os Pacotes **summarytools**, **DataExplorer**, **GGally**, **SmartEDA**, **psych**.
]
...
---
name: boas_praticas
## Boas práticas em AED

<br>

.alert2[
💡 A qualidade de uma **Análise Exploratória de Dados (AED)** depende tanto das ferramentas utilizadas quanto da **interpretação crítica** dos resultados.
]

<br>

.font95[
✔️ **Conheça seus dados** antes de construir modelos e usar técnicas multivariadas.

✔️ **Combine tabelas e gráficos** para compreender os dados.

✔️ **Investigue valores ausentes** e possíveis inconsistências.

✔️ **Correlação indica associação**, não relação de causa e efeito.

✔️ **Ferramentas automatizam análises. A interpretação continua sendo responsabilidade do analista.**
]
...
.center[
.font110[
🎯 **Uma boa AED produz conhecimento para orientar as próximas etapas da análise.**
]
]
